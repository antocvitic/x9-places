			...
			if (dowhat != null && dowhat.equals("addtags")
				...
				if (venueID != null && !venueID.isEmpty()) {
					...
					if (tempVenue == null) {
						...
						throw new RuntimeException("no venue with id "
								+ venueID + " to edit");
					...
					if (venuser.getID() != user.getID()
							&& venuser.getUsername() != null)
						venuser
								.applyReplevel(venuser.getReputationLevel() + 10);

					MessageDispatcher.sendMsgRedirect(req, resp,
							"/venue/view.jsp?venueID=" + venue.getID(),
							new OkMessage("The venue has been tagged."));
				...
				if (venueID != null && !venueID.isEmpty()) {
					...
					VenueModel tempVenue = VenueModel.getFromSolr(venueID);
					if (tempVenue == null) {
						throw new RuntimeException("no venue with id in Solr "
								+ venueID + " to edit");
					}
					if (UserUtils.getCurrentUser(req, resp)
							.getReputationLevel() < UserUtils.EDIT_LEVEL) {
						MessageDispatcher
								.sendMsgRedirect(
										req,
										resp,
										"/venue/view.jsp?venueID=" + venueID,
										new MessageDispatcher.ErrorMessage(
												"You do not have enough reputation points to edit this."));
					} else {
						builder = tempVenue.getEditable();
						builder.setTitle(title);
						builder.setAddress(address);
						builder.setDescription(description);
						// builder.setCreator(UserUtils.getCurrentUser(req,
						// resp));
						tagsList.addAll(tempVenue.getTags());
						builder.setTags(tagsList);
						VenueModel venue = builder.apply();
